...
test('My Planner Projects sample increments catalog and contributor counts', async ({ page, request }) => {
  const catalog = await fetchCatalog(request);
  await page.goto('./');
  ...
  await expect(page.getByRole('heading', { level: 3, name: 'My Planner Projects Agent' })).toBeVisible();
